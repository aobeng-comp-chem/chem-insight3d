...
class _OrbitalPickerDialog(QDialog):
    ...
    def _on_done(self, results):
        ...
        self.status_label.setText(
            f"✓  {len(results)} orbital grid(s) ready — loading into viewer…")
...
class MultiCubeVisualizer:
    ...
    # ── Feature 3: Isosurface statistics ──────────────────────────────────────
    ...
    def compute_isosurface_stats(self):
        ...
        vol_voxel  = dx * dy * dz          # Å³ per voxel
        iso = self.current_isovalue
    ...
    def _build_files_content(self, parent):
        ...
        for b in [open_btn, nbo_btn, save_btn,
...
# ── NBO / basis-file workflow ─────────────────────────────────────────────────
    ...
    def _load_computed_cubes(self, results):
        ...
        for r in results:
            try:
                # Convert result dict → cube dict (same format as read_cube)
                cube = self._result_to_cube(r)
                label = r['label']       # e.g. "molecule.31-7"
                idx   = len(self.cube_files)
                # Use label as the "filename" so the rest of the UI works
                self.cube_files.append(label)
                self.cubes.append(cube)
                # Tag this cube as unsaved so we can offer to save it later
                if not hasattr(self, '_unsaved_cubes'):
                    self._unsaved_cubes = {}
                self._unsaved_cubes[idx] = r   # keep result dict for saving
                if self.cube_list is not None:
                    self.cube_list.blockSignals(True)
                    item = QListWidgetItem(label + "  ●")   # ● = unsaved marker
                    item.setForeground(QBrush(QColor("#a6e3a1")))
